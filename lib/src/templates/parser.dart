...
class MarkdownDecoder extends Converter<String, Markdown> {
  ...
  Markdown convert(String input) {
    ...
    void maybeCommitParagraph() {
      ...
      paragraph.clear();
      blocks.addLast(MD$Paragraph(text: text, spans: _parseInlineSpans(text)));
    }
    ...
    for (var i = 0; i < length; i++) {
      ...
      if (line.isEmpty || _emptyPattern.hasMatch(line)) {
        ...
        // Parse headings
        final level = _headerPattern.firstMatch(line)?.group(0)?.length.clamp(1, 6) ?? 1;
        final text = line.substring(level).trim();
        pushBlock(MD$Heading(level: level, text: text, spans: _parseInlineSpans(text)));
        continue;
      } else if (line.startsWith('>')) {
        // Parse quotes
        ...
        final list = <({int intent, String marker, String text})>[
          (intent: 0, marker: marker, text: match.namedGroup('text')?.trim() ?? ''),
        ];
        var j = i + 1;
        for (; j < length; j++) {
          final line = lines[j];
          ...
          for (; offset < list.length; offset++) {
            ...
            if (item.intent == indent) {
              ...
              if (items.isNotEmpty) {
                ...
                items.last = items.last.copyWith(children: List<MD$ListItem>.unmodifiable(children));
              } else {
        ...
        MD$TableRow textToRow(String text) {
          ...
          return MD$TableRow(
            text: text,
            cells: List<List<MD$Span>>.unmodifiable(
              cells.sublist(1, cells.length - 1).map((cell) => cell.trim()).map(_parseInlineSpans),
            ),
          );
        }
        ...
        for (; j < length && lines[j].startsWith('|'); j++) rows.add(textToRow(lines[j]));
        // Validate
        final columns = header.cells.length;
        if (columns > 0 && separator && rows.every((row) => row.cells.length == columns)) {
          // All rows have the same number of cells as the header
          final text = lines.sublist(i, j).join('\n');
          pushBlock(MD$Table(text: text, header: header, rows: List<MD$TableRow>.unmodifiable(rows)));
        } else {
    ...
    return Markdown(markdown: input, blocks: List<MD$Block>.unmodifiable(blocks));
  }
}
...
final Uint8List _kind =
    Uint8List(2048)
      ..[42] =
          1 // * - italic and bold (single and double)
      ..[61] =
          1 // = - highlight (double)
      ..[95] =
          1 // _ - underline (double)
      ..[96] =
          1 // ` - monospace (single)
      ..[124] =
          1 // | - spoiler (double)
    ...
    void maybePushSpan(int end) {
      ...
      if (hasExcluded) {
        ...
        if (spanLength > 0) {
          ...
          spans.add(MD$Span(start: start, end: end - excluded.length, text: txt, style: mask));
        }
        ...
        spans.add(MD$Span(start: start, end: end, text: txt, style: mask));
      }
    }
